...
if (_uid != "76561198087347936" && _uid != "76561198130126033" && _uid != "76561198108525408") exitwith {
...
waitUntil {life_dog_order != 5};
		...
		{
				playSound3D ["\sounds\bark1.ogg", dog, false, getPosASL player, 1, 1, 225];
				life_dog_order = 5;
		};

		if (life_dog_order == 10) then 
		{
				playSound3D ["\sounds\bark2.ogg", dog, false, getPosASL player, 1, 1, 225];
				life_dog_order = 5;
		};

		if (life_dog_order == 11) then 
		{
				playSound3D ["\sounds\bark3.ogg", dog, false, getPosASL player, 1, 1, 225];
				life_dog_order = 5;
		};
		...
		{
				playSound3D ["\sounds\bark4.ogg", dog, false, getPosASL player, 1, 1, 225];
				life_dog_order = 5;
		};
		...
		if(_side countSide _nearestunits > 0) then
			...
			if ((dog distance getpos (_nearestunitofside select 0)) < 2 && (animationState (_nearestunitofside select 0)) != "BasicDriverOutDying") then 
			{
				...
				playSound3D ["\sounds\growl.ogg", dog, false, getPosASL player, 1, 1, 225];
				uiSleep 4;
