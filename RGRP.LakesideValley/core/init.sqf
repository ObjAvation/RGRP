...
A3L_TFEnabled = true;
A3L_onTsServer = "[RGRP] Ritality Gaming Roleplay" == (call TFAR_fnc_getTeamSpeakServerName);
...
[] spawn {
	...
	while {true} do {
				...
				if (!(_isadmin)) then {
					...
					_onTsServer = "[RGRP] Ritality Gaming Roleplay" == (call TFAR_fnc_getTeamSpeakServerName);
					if (!(_onTsServer)) then {
